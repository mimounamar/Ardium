...
public class DiamondBush extends Block
...
public DiamondBush(Item item)
{
  ...
  this.textureNames[0] = "Diamond_Bush_1";
  this.textureNames[1] = "Diamond_Bush_2";
}
...
public boolean onBlockActivated(World world, int x, int y, int z, EntityPlayer player, int par6, float par7, float par8, float par9)
{
  ...
  if (meta >= 1)
  {
    ...
    world.setBlock(x, y, z, this, 0, 3);
    world.spawnEntityInWorld(new EntityItem(world, player.posX, player.posY, player.posZ, new ItemStack(Items.diamond)));
    return true;
  }
  return false;
}
...
{
  return Item.getItemFromBlock(Ardium.dbush);
}
...
public void registerBlockIcons(IIconRegister icon)
{
  ...
  for (int i = 0; i < this.fastIcons.length; i++) {
    if (this.textureNames[i] != "")
    {
      ...
      this.icon = icon.registerIcon("ardium:Diamond_Bush_2_Fast");
    }
  }
}
